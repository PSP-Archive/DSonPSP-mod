...
/*
void dsonpsp_init(BOOL firstTime)
{
}
*/
int main(void)
...
/*	not needed for now, nds deinit is not working and i need to know why

  if(pspSdkLoadStartModule("SystemButtons.prx", PSP_MEMORY_PARTITION_KERNEL) < 0){
    SceCtrlData pad;
		printf("Error loading SystemButtons.prx");
		sceKernelDelayThread(100000);
		do { sceCtrlReadBufferPositive(&pad, 1);
				} while (pad.Buttons == 0);
		sceKernelExitGame();
  }
  initSystemButtons(sceKernelDevkitVersion());
*/
...
/*
	if (!firstTime){
		if (cflash_disk_image_file)
			free(cflash_disk_image_file);
	}
*/
