...
//   noext;
   ...
   ROMReader_struct * reader = ROMReaderInit(&noext);
   int type = ROM_NDS;
...
#endif
	 FILE * file = reader->Init(filename);
...
#ifdef __psp__
	int i = reader->Read(file, data, 0x4000);
#else
   int i = reader->Read(file, data, size);
#endif
   reader->DeInit(file);
	//Rom Decryption 
